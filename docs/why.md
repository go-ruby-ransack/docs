...
`go-ruby-ransack/ransack` reimplements Ruby's `ransack` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-ransack/ransack` directly, with no Ruby runtime;
...
Because `ransack` is pure Go with cgo disabled, it also compiles to **WebAssembly**
